...
import { generateF5TTS, getDemoVoices, subscribeToTtsProgress, uploadVoice, recordVoice, downloadYouTubeVoice, transcribeReferenceAudio } from '../services/api'
...
// Pro F5TTS Czech použijeme prefix f5tts_cs_ pro oddělení od slovenského
const getVariantStorageKey = (voiceId, variantId) => `f5tts_cs_voice_${voiceId}_variant_${variantId}`
...
function F5TTSCzech({ text: textProp, setText: setTextProp, versions, onSaveVersion, onDeleteVersion }) {
  ...
  const makeDemoRefKey = (voiceId) => `f5tts_cs_reftext:v1:${language}:demo:${voiceId}`
  const makeUploadRefKey = (filename) => `f5tts_cs_reftext:v1:${language}:upload:${filename}`
  ...
  const handleCopySettings = () => {
    try {
      ...
      sessionStorage.setItem('tts_copied_settings', JSON.stringify(settingsToCopy))
      console.log('✅ ÚSPĚŠNĚ ULOŽENO DO SESSIONSTORAGE!')
      console.log('📋 Nastavení zkopírována z varianty:', activeVariant)

      // Ověření
      const verify = sessionStorage.getItem('tts_copied_settings')
  ...
  const handlePasteSettings = () => {
    try {
      ...
      if (voiceId) {
        ...
        console.log('💾 Klíč:', `f5tts_cs_voice_${voiceId}_variant_${activeVariant}`)
      }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const ttsParams = {
        ...
        // Volitelný přepis referenčního audia
      ...
      // F5TTS Czech/Slovak endpoint
      const result = await generateF5TTS(text, voiceFile, demoVoice, ttsParams, jobId)
  ...
  return (
    <>
        ...
        <div className="generate-content">
          <div className="section-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              <h2>F5-TTS {useSlovakModel ? '(Slovak model + CZ fonetika)' : '(České slovo)'}</h2>
              <button
          ...
          {/* Slovak Model Toggle */}
          <div style={{
            background: useSlovakModel ? 'rgba(76, 175, 80, 0.15)' : 'rgba(0, 0, 0, 0.2)',
            border: `1px solid ${useSlovakModel ? 'rgba(76, 175, 80, 0.3)' : 'rgba(255, 255, 255, 0.1)'}`,
            borderRadius: '12px',
            padding: '16px',
            marginBottom: '16px'
          }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                >
                  <div style={{
                    ...
                    width: '48px',
                    height: '24px',
                    background: useSlovakModel ? '#4caf50' : 'rgba(255,255,255,0.2)',
                    borderRadius: '12px',
          ...
          <div className="generate-section">
            ...
            >
              {loading ? 'Generuji...' : useSlovakModel ? 'Generovat řeč (Slovak model + CZ fonetika)' : 'Generovat řeč (F5-TTS Czech)'}
            </Button>
          </div>
      ...
      <HelpSidebar
        ...
        title="Nápověda - F5-TTS (České slovo)"
      >
        <F5TTSHelpContent />
      </HelpSidebar>
    </>
