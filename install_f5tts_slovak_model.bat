...
REM Skript pro stahování slovenského F5-TTS modelu z Hugging Face
...
echo ========================================
echo F5-TTS Slovak Model Download
echo ========================================
echo.
...
echo.
echo Spoustim stahovani slovenskeho F5-TTS modelu...
echo Model: petercheben/F5_TTS_Slovak
echo Velikost: ~1.35 GB
echo.
echo POZOR: Stahovani muze trvat nekolik minut podle rychlosti internetu.
echo.

if %PYTHON_IS_FILE%==1 (
    "%PYTHON_EXE%" "%ROOT%install_f5tts_slovak_model.py"
) else (
    %PYTHON_EXE% "%ROOT%install_f5tts_slovak_model.py"
)
...
if errorlevel 1 (
    ...
    echo 2. Zkontrolovat dostupne misto na disku (potrebujete alespon 2 GB)
...
echo.
echo ========================================
echo F5-TTS Slovak model download completed!
echo ========================================
echo.
echo Model je nyni pripraven k pouziti.
echo.
pause
exit /b 0
